...
class vb_raggruppa_documenti(osv.osv_memory):
    ...
    _columns = {
        'nuova_causale': fields.many2one('vendita.causali', 'Causale',
                                         required=True),
        # ----- Dati DDT
    ...
    def onchange_causale(self, cr, uid, ids, causale, context):
        if causale and self.pool.get('vendita.causali').browse(cr,
                                                               uid,
                                                               causale).ddt:
            vb = self.pool.get('vendita_banco').browse(cr, uid,
                                                       context['active_id'])
            return {'value': {
                'goods_description_id': (
                    vb.partner_id.goods_description_id and
                    vb.partner_id.goods_description_id.id or False),
                'transportation_reason_id': (
                    vb.partner_id.transportation_reason_id and
                    vb.partner_id.transportation_reason_id.id or False),
