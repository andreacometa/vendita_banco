...
class vb_raggruppa_fatture(osv.osv_memory):
	...
	_columns = {
		...
		'ordini_ids' : fields.one2many('vb.ordini_da_fatturare', 'vb_raggruppa_fatture_id', 'Documenti da fatturare'),
		'raggruppa' : fields.boolean('Raggruppa', help="Raggruppa gli ordini dello stesso cliente e crea un'unica fattura."),
	...
	def genera_fatture_raggruppate(self, cr, uid, ids, doc_objs, context={}):
		...
		for doc in doc_objs:
			...
			self.pool.get('vendita_banco').write(cr, uid, [order_obj.id], 
...
class vb_ordini_da_fatturare(osv.osv_memory):
	...
	_columns = {
		...
		'partner_id' : fields.related('name', 'partner_id', 
			type="many2one", relation="res.partner", string='Cliente'),
		'totale' : fields.related('name', 'totale', type="float", string='Totale'),
		'imponibile' : fields.related('name', 'imponibile', type="float", string='Imponibile'),
		'vb_raggruppa_fatture_id' : fields.many2one('vb.raggruppa_fatture', 'Raggruppa'),
		'modalita_pagamento_id' : fields.related('name', 'modalita_pagamento_id', 
			type="many2one", relation="account.payment.term", string='Modalità di pagamento'),
		}
