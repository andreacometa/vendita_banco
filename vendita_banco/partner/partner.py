...
class res_partner(osv.osv):
    ...
    def _vb_credit(self, cr, uid, ids, name, arg, context=None):
        ...
        for id in ids:
            tot = 0.0
            vb_ids = vb_obj.search(cr, uid, [
                ('state', '=', 'draft'), ('partner_id', '=', id)])
            if vb_ids:
    ...
    _columns = {
        'causale_id' : fields.many2one('vendita.causali', 'Causale', help="Causale predefinita da usare nella vendita al banco"),
        'tipo_trasporto_id' : fields.many2one('vendita_banco.trasporto', 'Tipo Trasporto', help="Tipo Trasporto predefinito da usare nella vendita al banco"),
